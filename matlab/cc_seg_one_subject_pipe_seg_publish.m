function [varargout] = cc_seg_one_subject_pipe_seg_publish(FileNameOrStruct, GroundTruthFile, OutputDir, OutputPrefix)
...
FileNameOrStruct = 'oasis_database/flipped/OAS1_0068_msp.nii.gz';
GroundTruthFile = 'oasis_database/flipped/OAS1_0068_cc.nii.gz';
OutputDir = 'oasis_publish';
OutputPrefix = '0045-OAS1_0068_msp';
FileNameOrStruct = 'datasets/mindboggle101/HLN-12-4.nii.gz';
GroundTruthFile = [];
OutputDir = 'mindboggle_junk';
OutputPrefix = '0008-HLN-12-4';
%% load data
...
subplot(SR, SC, 2);
imshow(ProbAVW, []);
title('CC probability map');
subplot(SR, SC, 3);
imshow(FornixProbAVW, []);
title('Fornix probability map');

%% resample image to template pixel size, crop templates
...
%% get initial otsu segmentation for cross correlation
...
%% smooth white matter segmentation and run normalised cross correlation
...
title('White matter segmentation smoothed');
subplot(SR, SC, 2);
imshow(MatchedFilterProb, []);
title('CC Probability map');
subplot(SR, SC, 3);
...
%% get regional maxima of cross correlation image
...
%[I, J] = find
hold on;
plot(cc2RRegMaxJ, cc2RRegMaxI, 'y*');
%plot(Offsets(BestLKI, 2) + SZ(2), Offsets(BestLKI, 1) + SZ(1), 'r*');
title('Regional maxima of cc2R');
	
%% Lucas-Kanade tracker and best cross-correlation maxima selection
% Lucas-Kanade tracker is a fast affine registration algorithm
% it requires a good initial estimate so we try a few different initialisation points
% we try the closest regional maxima to where we expect the CC to be  (in the centre of the image)
% then we try the two highest regional maxima of the cross correlation image 
...
for MainFittingIteration = 1:2
	...
	% so new strategcc_seg_one_subject_pipe_seg_publish.my, try the closest and if this fails (OtsuMask is empty) then resort to the CC values
...
%% make initial guess and get intensity distribution of pixels in guess
...
subplot(SR, SC, 2);
imshow(FirstStartMask, []);
title('Initial guess');
subplot(SR, SC, 3);
histfit(ResampledAVWCropped(FirstStartMask));
title('Gaussian fit to histogram');
subplot(SR, SC, 4);
imshow(GaussianProb, []);
title({'Probability of each pixel based on fitted Gaussian', 'Intensities above the mean are given high probability'});
...
%% make exclusion weight map
% make a "penalty" image where a higher value indicates less probability of belonging to the CC

SIGMA = 0.25;

[GaussianFilter, GaussianFilterDeriv] = gaussian_filter_max_1d(SIGMA);

aSmooth=imfilter(ResampledAVWCropped, GaussianFilter, 'conv', 'replicate', 'same');   % run the filter across rows
...
ExclusionC = exp(-2 .* GaussianProb .* (TemplateProbLKIMGCropped + 5));
...
subplot(SR, SC, 1);
imshow(ExclusionA, []);
title('Fornix Prob * Intensity ');
subplot(SR, SC, 2);
imshow(ExclusionB, []);
title('Gradient strength on Gaussian probability image');
subplot(SR, SC, 3);
imshow(ExclusionC, []);
title('Penalty on low probability intensities');
subplot(SR, SC, 4);
imshow(ExclusionWeight, []);
title('Exclusion map (end result)');

%% retain connected components that are in the low penalty region, only keep connected components bigger than 200 pixels
...
subplot(SR, SC, 4);
imshow(StartMask, []);
title('Retained, bigger than 200');
...
%% fill holes in start mask and join up regions if there are more than one region
% Use minimum spanning tree algorithm if there are more than 2 regions
% but constrain joining segments to be in high gaussian probability regions
...
%% if mask is still not joined, join again without constraints
...
%% close mask to smooth boundaries
clf;
SR = 1;
SC = 2;
subplot(SR, SC, 1);
...
%% recompute Gaussian fit with new mask and mask out low probability pixels, retain only largest connected component, fill holes
...
%% perform filling on exclusion mask using morphological reconstruction
...
imshow(ResampledAVWCropped, []);
hold on;
[~, CC] = contour(StartMaskBeforeReconstruct, [0.5, 0.5]);
set(CC, 'Color', 'b');
[~, CC] = contour(StartMask, [0.5, 0.5]);
set(CC, 'Color', 'r');
title('Original (blue) + Reconstructed (red)');
