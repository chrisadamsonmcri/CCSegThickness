...
FID = fopen('cc_seg_mindboggle.parallel', 'w');
%Output
for z = 1:length(D)
	...
	fprintf(FID, 'matlab -nodesktop -nosplash -r "cc_seg_one_subject_pipe(''%s'', '''', ''%s'', ''%s''); exit;"\n', ...
		fullfile('datasets', 'mindboggle101', D(z).name), ...
		OutputDir, ...
