function [UsedConvexHull] = cc_seg_one_subject_final(InFile, GroundTruthFile, OutputDir, OutputPrefix)
...
if(all(Overlaps < 0.975))
	%clf;
	%imshow(label2rgb(DMaskL, 'lines', 'k'), []);
	disp('overlaps dont match, this shouldnt happen');
	return;
	%keyboard;
else
	[~, L] = max(Overlaps);
...
[NewPSI, NumIter] = fls_evolution_illusory_contours_cc_seg(-D, OriginalPSI, ones(size(D)), zeros(size(D)), [1, 1], 1000, 200, 200, 5, ResampledAVWCropped);
...
OutputS.FinalSeg = FinalSegOpenedM;
