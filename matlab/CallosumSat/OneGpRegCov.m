function OneGpRegCov(xlsfiles, numCov, threshold, loop)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%function OneGpRegCov(xlsfiles, numCov, threshold, loop)
% e.g. OneGpRegCov({'xlsfile'}, numCov, 0.05, 10e3)
%
% Input:
%       xlsfile     xls file of one group with covariates at first n column, 
%                   first column is regressor and rest n-1 column is cov       
%       threshold   threshold of P, default is 0.05
%       loop        number of randomisation trials, default is 10e3
% 
%
%This function looked at regression on the first column (first cov) and covary for second cloumn (second cov)?
%
% Jian Chen 08/09/08
...
%FULL MODEL WITH TWO COVS
InvF=pinv(DesMat);

%REDUCED MODEL ONLY WITH SECOND COV
DesRedMat = DesMat;
DesRedMat(:,2)=[];
InvRGp=pinv(DesRedMat);
...
for count=numCov+1:m
  ...
  SSERGp=(Mat(:,count))'*Mat(:,count)-(BRedGp(:,count-numCov)'*DesRedMat'*Mat(:,count));
...
for lcount=1:loop
  ...
  BRandRedGp=zeros(size(DesRedMat,2),slices);
  ...
  for countR=numCov+1:m
    ...
    SSERGp=(MatR(:,countR))'*MatR(:,countR)-(BRandRedGp(:,countR-numCov)'*DesRedMat'*MatR(:,countR));
