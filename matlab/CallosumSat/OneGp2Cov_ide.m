function OneGp2Cov_ide(xlsfiles, threshold, loop)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%function OneGp2Cov_ide(xlsfile, threshold, loop)
% e.g. OneGp2Cov_ide({'xlsfile'}, 0.05, 10e3)
% This program test if the regression models yield same responds function
% i.e. the coefficients of ordinal cov and interaction of it with
% quatitative cov are all zero
%
% Kutner & Neter, 5 ed, chapter 8, p327
%
% Input:
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%CREATE DESIGN MATRIX FOR REGRESSION MODEL WITH INTERACTIONS
...
%REDUCED MODEL HAS TWO COEFFICIENT (NO INTERACTION & TWO REGRESSION SAME )
BRedGp=zeros(size(DesMat,2)-2,slices);
...
%REDUCED MODEL (TEST FOR THE COEFFICIENTS OF ORDINAL COV AND INTERACTION EQUAL ZERO)
% (chapter 8, p327)
InvRGp=pinv(DesMat(:,[1, 3]));
...
for count=numCov+1:m
  ...
  SSERGp=(Mat(:,count))'*Mat(:,count)-(BRedGp(:,count-numCov)'*DesMat(:,[1, 3])'*Mat(:,count));
...
for loopcount =1:loop
  ...
  BRandRedGp=zeros(size(DesMat,2)-2,slices);
  ...
  for countR=numCov+1:m
    ...
    SSERGp=(MatR(:,countR))'*MatR(:,countR)-(BRandRedGp(:,countR-numCov)'*DesMat(:,[1,3])'*MatR(:,countR));
