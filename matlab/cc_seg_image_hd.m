...
OutputDir = 'cc_seg_none_test';

D = dir(fullfile(OutputDir, '*_thickness.hdf5'));

[ThicknessFiles{1:length(D)}] = deal(D.name);

Prefixes = strrep(ThicknessFiles, '_thickness.hdf5', '');

SubjectIDs = cell(size(Prefixes));
TimePoints = zeros(size(Prefixes));

TimeStrings = {'SRT1', 'SRT2', 'SRT3'};
NumTimePoints = length(TimeStrings);

ThicknessProfiles = cell(size(Prefixes));

for z = 1:length(Prefixes)
	T = strsplit(Prefixes{z}, '-');
	SubjectIDs{z} = str2double(T(3:4));
	[~, TimePoints(z)] = ismember(T{end}, TimeStrings);
	%keyboard;
	%I = h5info(fullfile(OutputDir, ThicknessFiles{z}));
	ThicknessProfiles{z} = h5read(fullfile(OutputDir, ThicknessFiles{z}), '/registeredThicknessProfile');
end

ThicknessProfiles = cat(2, ThicknessProfiles{:});
SubjectIDs = cat(1, SubjectIDs{:});

[SubjectIDs, ~, J] = unique(SubjectIDs, 'rows');

CThicknessProfiles = NaN([size(ThicknessProfiles, 1), length(SubjectIDs), 3]);
for z = 1:length(Prefixes)
	CThicknessProfiles(:, J(z), TimePoints(z)) = ThicknessProfiles(:, z);
end

ThicknessProfiles = CThicknessProfiles;
clear CThicknessProfiles;
...
for z = 1:length(NeuropsychData.ID)
	NeuropsychData.ID{z} = str2double(strsplit(NeuropsychData.ID{z}, '.'));
end
NeuropsychData.ID = cat(1, NeuropsychData.ID{:});

%keyboard;
GroupNames = {'CTL', 'PRE', 'SYM'};

% assign the groups to the IDs
TF = ismember(NeuropsychData.ID, SubjectIDs, 'rows');
...
[UniqueNeuropsychIDs, ~, J] = unique(NeuropsychData.ID, 'rows');
...
for CurGroup = 1:3
	...
	for CurTimePoint = 1:3
		T(:, CurTimePoint) = nanmean(ThicknessProfiles(:, NeuropsychData.Group == CurGroup, CurTimePoint), 2);
	end
...
% nan out the thickness profiles that we don't have neuropsych for
[ID, JD] = find(isnan(NeuropsychData.Scan_date_and_age.AGE));

for z = 1:length(ID)
	ThicknessProfiles(:, ID(z), JD(z)) = NaN;
end
...
for z = 1:length(LaurenData.IMAGEHD_CODE)
	LaurenData.IMAGEHD_CODE{z} = str2double(strsplit(LaurenData.IMAGEHD_CODE{z}, '.'));
end
LaurenData.IMAGEHD_CODE = cat(1, LaurenData.IMAGEHD_CODE{:});

[~, LOC] = ismember(LaurenData.IMAGEHD_CODE, NeuropsychData.ID, 'rows');
