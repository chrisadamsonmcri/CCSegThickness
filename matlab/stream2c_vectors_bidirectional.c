...
#define	SX_IN	    prhs[4]
#define	SY_IN	    prhs[5]
#define	STEP_IN	    prhs[6]
#define	MAXVERT_IN  prhs[7]
...
{
	double *x, *y, *u, *v, *sx, *sy, *step, *maxVert;
	int xSize, ySize;
	const int *dims;
	int numVerts;
	int i, NumPoints;
	...
	if (nrhs != 8)
	{
		mexErrMsgTxt("mystream2c_vector requires 8 input arguments. [verts] = stream3c(x,y,u,v,sx,sy,step,maxVert)");
	}
	else if (nlhs != 1) 
	{
		mexErrMsgTxt("mystream2c_vector requires 1 output arguments. [verts] = stream3c(x,y,u,v,sx,sy,step,maxVert)");
	}
	...
	for (i = 0; i < NumPoints; i++)
	{
		...
		if(x != NULL && y != NULL)
		{
			numVerts = traceStreamXYUV(x, y, u, v, xSize, ySize, sx[i], sy[i], *step, (int)(*maxVert));
		}
		else
		{
			numVerts = traceStreamUV(u, v, xSize, ySize, sx[i], sy[i], *step, (int)(*maxVert));
		}
		if (verts)
		{
