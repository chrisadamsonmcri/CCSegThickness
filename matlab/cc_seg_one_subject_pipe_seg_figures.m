...
T = padarray(ResampledAVW, PS, 0, 'both');
...
title([roman_label(1), ' {\itInputIMG}'], TitleProps{:});
...
imshow(T, []);
rectangle('Position', [PS(2), PS(1), size(WMSegSmoothed, 2), size(WMSegSmoothed, 1)], RecProps{:});
title([roman_label(2), ' {\itWMSeg}'], TitleProps{:});
...
imshow(T, []);
hold on;
title([roman_label(3), ' {\itNCC}'], TitleProps{:});
...
PA = plot(xoffset2R(OffsetValid2R(F(1))) + size(MatchedFilter, 2), yoffset2R(OffsetValid2R(F(1))) + size(MatchedFilter, 1), 'wx', 'LineWidth', 2, 'MarkerSize', 10);
PB = plot(xoffset2R(OffsetValid2R(F(2))) + size(MatchedFilter, 2), yoffset2R(OffsetValid2R(F(2))) + size(MatchedFilter, 1), 'wx', 'LineWidth', 2, 'MarkerSize', 10);
PC = plot(xoffset2R(OffsetValid2R(F(3:end))) + size(MatchedFilter, 2), yoffset2R(OffsetValid2R(F(3:end))) + size(MatchedFilter, 1), 'wo', 'LineWidth', 2);

PD = plot(xoffset2RReg + size(MatchedFilter, 2), yoffset2RReg + size(MatchedFilter, 1), 'w+', 'MarkerSize', 10, 'LineWidth', 2);
axis on;
set(gca, 'XTick', [], 'YTick', []);

for z = 2:3
	LeftAXPos = get(AX(z - 1), 'Position');
	RightAXPos = get(AX(z), 'Position');
	RightAXPos(1) = LeftAXPos(1) + LeftAXPos(3) + 0.01;

	set(AX(z), 'Position', RightAXPos);
end
...
ylabel('{\itNCC}', 'Rotation', 270, 'VerticalAlignment', 'bottom');
%keyboard;
...
OutputFile = fullfile(OutputDir, 'paper_figures', [OutputPrefix '-001-NCC-ismrm.eps']);
set([PA(:); PB(:); PC(:); PD(:)], 'MarkerSize', 12.5, 'Color', 'r');
%cc_seg_save_figure_paper_eps(OutputFile);
cc_seg_save_figure_paper_png(OutputFile);
...
%  		FinalSegOpenedL = labelmatrix(FinalSegOpenedCC);
	...
	%[~, I] = max(FinalSegOpenedAreas);
	%FinalSegOpenedM = FinalSegOpenedL == I;
	FinalSegOpenedM = FinalSegOpened;
...
%		SR = 1;%
	...
	% 	AX(3) = subplot(SR, SC, 3);
		...
		T(A & ~B) = 2;
		T(~A & B) = 1;
		% Addition suggested by richard, only have intersection of A & B
		T = double(T == 3);
		...
		LegHandle = text(0.4, 0.25, {'Legend:', '        White - {\itLKCCSeg} \cap {\itPCCMask}', '        BoundaryTangent({\itPCCMaskBP})', '        BoundaryTangent({\itLKCCSegBP})'}, ...
			'Units', 'Normalized', ...
			'Color', 'w', ...
			'HorizontalAlignment', 'left', ...
			'FontSize', 14);
		...
		GG = 54.7;
		arrow([70.6, GG], [74.6, GG], 10, 'LineWidth', 2, 'EdgeColor', 'r', 'FaceColor', 'r', 'BaseAngle', 30);
		%set(QR, 'Color', 'r');
		arrow([70.6, GG + 3.6], [74.6, GG + 3.6], 10, 'LineWidth', 2, 'EdgeColor', 'b', 'FaceColor', 'b', 'BaseAngle', 30);
